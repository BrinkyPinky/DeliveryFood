...
struct OrderConfirmView: View {
    @ObservedObject private var viewModel: OrderConfirmViewModel
    @Binding var isOrderConfirmPresented: Bool
    
    init(totalPrice: Double, isOrderConfirmPresented: Binding<Bool>) {
        viewModel = OrderConfirmViewModel(totalPrice: totalPrice)
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                Form {
                    Section("Address") {
                        NavigationLink(viewModel.currentPickedAddressModel?.addressName ?? "Pick the address") {
                            AddressSelectionView(selectedAddressModel: $viewModel.currentPickedAddressModel)
                        }
                    }
                ...
                TabBarForOrderConfirmView(viewModel: viewModel, isOrderConfirmPresented: $isOrderConfirmPresented)
            }
            .padding([.leading, .trailing], 32)
        }
...
#Preview {
    ...
    .sheet(isPresented: .constant(true), content: {
        OrderConfirmView(totalPrice: 8.50, isOrderConfirmPresented: .constant(true))
    })
}
    ...
    var body: some View {
        VStack {
            ...
            Button {
                ...
                .animation(.easeInOut(duration: 0.01), value: viewModel.isPriceLoading)
                .padding()
    ...
    var body: some View {
        Section("Delivery Details") {
            ...
            if viewModel.deliveryTimeType == .onTime {
                DatePicker("", selection: $viewModel.selectedDate, in: viewModel.dateRange, displayedComponents: .hourAndMinute)
