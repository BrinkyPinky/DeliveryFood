...
final class AccountViewModel: ObservableObject {
    ...
    func onAppearAction() {
        ...
        FirebaseFirestoreManager.shared.getPreviousOrdersForUserID(userID) { [weak self] result in
            guard let self = self else { return }
            
            switch result {
            case .success(let previousOrderModels):
                self.previousOrderModels = previousOrderModels
    ...
    // Декодирует информацию о заказе
    func orderStatusDecoder(_ orderStatus: Int) -> String {
        switch orderStatus {
        case 0: "Ordered"
        case 1: "Accepted"
        case 2: "Cooking"
    ...
    func saveDataChangesAction() {
        ...
        FirebaseFirestoreManager.shared.changeUserData(userID: userID, name: userName, phoneNumber: userPhoneNumber) { [weak self] result in
            guard let self = self else { return }
    ...
    func loadUserData() {
        ...
        FirebaseFirestoreManager.shared.getUserDataByUserID(userID) { [weak self] result in
            guard let self = self else { return }
