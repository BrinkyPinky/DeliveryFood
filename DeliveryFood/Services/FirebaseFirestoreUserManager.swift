...
final class FirebaseFirestoreUserManager {
    static var shared = FirebaseFirestoreUserManager()
    ...
    func getFood(completion: @escaping (Result<[DetailFoodModel], Error>) -> ()) {
    ...
    func placeAnOrder(userID: String, paymentType: String, date: Date, address: AddressModel, totalPrice: Double, cartPositions: [PositionForBillModel], completion: @escaping (Result<Any?, Error>) -> ()) {
        ...
        // Получение ссылок на заказы
        let orderByUserRef = db.collection("Orders").document("OrdersByUsers").collection(userID).document()
        
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
        let dateString = dateFormatter.string(from: date)
        
        let orderByDateRef = db.collection("Orders").document("OrdersByDates").collection(dateString).document(orderByUserRef.documentID)
        
        let orderData: [String:Any] = [
            "date":date,
            "userID":userID,
            "orderID":orderByUserRef.documentID,
            ...
            "positions":positions,
            "ordersByUsersRef":orderByUserRef.path,
            "ordersByDatesRef":orderByDateRef.path,
            "orderStatus":0
        ]
        
        db.runTransaction { transaction, errorpointer in
            transaction.setData(orderData, forDocument: orderByUserRef)
            transaction.setData(orderData, forDocument: orderByDateRef)
    ...
    func getPreviousOrdersForUserID(_ userID: String, completion: @escaping (Result<[PreviousOrderModel], Error>) -> ()) {
        listener = db.collection("Orders").document("OrdersByUsers").collection(userID).addSnapshotListener({ snapshot, error in
            if error != nil {
                completion(.failure(FirebaseFirestoreError.getDataError))
            }
